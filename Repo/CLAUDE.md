...
1. **`/docs/DESIGN_SYSTEM.md`** (v1.1) — all visual design decisions. Read this before any UI component, page, or styling work. Non-negotiable. Pay special attention to section 4.1 (exact spacing allowlist), section 5.4 (tag collapse rule), section 7.5 (English vs Persian strings), and the "Component creation order" rule near the top.
2. **`/docs/PROJECT_PLAN.md`** — the 8-week sprint plan, database schema, and feature priorities. Read this to understand sprint scope before starting any week's prompts.
