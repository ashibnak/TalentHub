...
- **v1.1** (current) — پچ‌های حاصل از `dry-run audit` با `Claude Code`: قانون ترتیب ساخت component (`component creation order`)، لیست دقیق کلاس‌های مجاز spacing، قانون collapse برای لیست تگ‌ها، و راهنمای صریح «کدام رشته‌ها فارسی بمونن، کدام‌ها انگلیسی»
- **v1.0** — اولین نسخه‌ی کامل: palette نهایی (بدون cream)، typography scale (vibrant tech با Display 56)، dark mode فقط، component conventions کامل (button، input، badge، avatar، card، pill، spotlight)، layout patterns، Persian/RTL rules، Lucide icon system با stroke-width 1.5، anti-patterns list
...
### Component creation order (mandatory)

If a page needs a component that doesn't exist yet in `/components/atoms/` or `/components/molecules/`, **create it as a separate file first**, then import it. Never define a component inline inside a page file — not even a "temporary" one, not even for a one-off test page.

```
❌ Wrong: defining <StatusPill /> as a local function inside /app/u/[username]/page.tsx
✅ Right: create /components/atoms/StatusPill.tsx, export it, import it into the page
```

This applies even when building a brand-new page that seems to need brand-new pieces — check the folder first, and if the piece belongs to a reusable category (a status indicator, a card, a tag, a badge), it goes in `/components/atoms/` or `/components/molecules/` before it's used anywhere.
...
**Allowed Tailwind spacing classes (padding, margin, gap — any direction/axis):**
...
(and their directional variants: `px-`, `py-`, `pt-`, `pb-`, `ps-`, `pe-`, `mx-`, `my-`, etc. — same numeric suffixes only)

**Explicitly forbidden — do not use, even if they seem "in between":**

```
p-2.5 / p-3.5 / p-5 / p-7 / p-9 / p-10 / p-11 / p-14 / p-20 / p-24
gap-2.5 / gap-3.5 / gap-5 / gap-7 / gap-9 / gap-10
```

If a spacing value feels like it should be "between" two allowed values, round to the nearest allowed one — don't introduce a new one. This is a hard rule, not a guideline: `gap-2.5` (10px) is not an acceptable substitute for `gap-3` (12px), even though it looks close.
...
**Tag collapse convention (mandatory for any list of skills/tags):**

Whenever rendering a list of skill tags, AI tool tags, or domain tags on a card or profile, **never render the full list unbounded**. Default to showing the first 6 tags, then a collapse chip for the rest:
...
// ...render visible tags, then if hiddenCount > 0:
<span className="bg-white/6 text-text-muted text-micro px-2 py-0.5 rounded-sm">
  +{hiddenCount}
</span>
```

This applies to: profile skill tags, profile domain expertise tags, project skill/tool tags in cards. It does not apply to the full profile edit form (where all selected tags should be editable/removable) — only to **display/read** contexts.
...
### 7.5 Which strings stay English vs become Persian (mandatory reference)

This is the single most common source of inconsistency — follow this table exactly, don't decide per-prompt.

**Stay English, always:**
- Status pill labels: `Currently building`, `Shipped`, `Idle`
- Project stage labels: `Experiment`, `Weekend Hack`, `Building`, `Shipped`, `Maintained`
- AI tool names: `Cursor`, `Claude Code`, `Lovable`, `v0`, `Bolt`
- Technical skill/tag names: `React`, `Python`, `PostgreSQL`, etc. (these come from the `skills` seed data, which is English)
- Special badge labels: `Spotlight`, `AI Verified`
- Role badge labels: `Builder`, `Domain Expert`, `Hybrid`
- Admin table column headers (e.g., in `/app/admin/users`)
- Any string that is a proper noun or a fixed platform vocabulary term

**Become Persian, always:**
- All button labels (e.g., دنبال کن, ثبت پروژه, ادامه)
- All body text, descriptions, bios (unless the user wrote their own bio in English)
- All headings and page titles (e.g., چالش‌های هفته)
- All form labels and placeholders
- All toast/error/success messages
- All empty-state copy
- All onboarding wizard copy
- Timestamps in relative Persian context (e.g., ۲ روز پیش) — but see 7.4 for when English "2 days ago" is acceptable (dense metadata rows)

**When in doubt:** platform vocabulary (words that name a *feature or mechanic* of AIGraph itself) stays English. Words that are *instructions or descriptions to the user* become Persian. This mirrors how "Spotlight" and "Shipped" function like product terminology (similar to how "Pull Request" stays English even in a Persian-speaking dev's vocabulary), while "توضیح بده چرا این پروژه به این مشکل می‌خوره" is conversational guidance and must be Persian.
