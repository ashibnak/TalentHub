...
**Version: v0.5** · phase 1 (internal launch) — pre-build

## Changelog

- **v0.5** (current) — Domain Expert persona اضافه شد: schema جدید `domains` و `user_domain_expertise`، logic `role_badge` (Builder/Domain Expert/Hybrid)، section "Domain Experts" روی Challenge detail page، endorsement-based verification موکول به فاز ۲
...
اگه آخر هفته ۸، **۵۰ نفر active، ۷۵ پروژه، ۷۰٪ verification rate، ۲۵ نفر "Currently Building"، spotlight اول با ۱۲+ submission، ۵+ پیشنهاد Problem از community، و ۸+ پروفایل Domain Expert** داری، MVP موفقه و آماده‌ی scale به ۲۵۰۰ نفر تو ماه ۳-۴ هستی.

اگه spotlight اول کمتر از ۱۲ submission گرفت یا community هیچ Problem پیشنهاد نداد، **قبل از scale، loop رو fix کن**. اگه ۸ Domain Expert profile نداشتی، یعنی این persona جذب نشده — یا onboarding پیامش واضح نبود برای non-technical ها، یا UI کاربر non-builder رو می‌ترسونه. این هم سیگنال product-market-fit ست.
